...
def diff_bc(ndim, bc_node_list, x_all, prob: str, t=None):
    ...
    if prob == 'diff-test' and ndim == 3:
        for ibc in range(1):
            ...
            for inod in range(bci.shape[0]):
                ...
                u_bc[0][inod // nloc, inod % nloc] = torch.exp(-x-y-z)
        ...
        for ibc in range(1, len(bc_node_list)):
            ...
            for inod in range(bci.shape[0]):
                ...
                y = x_inod[1]
                # unsymmetry stress formulation
                u_bc[ibc][inod // nloc, inod % nloc] = -torch.exp(-x-y-1)
        for inod in range(nonods):
            x_inod = sf_nd_nb.vel_func_space.x_ref_in[inod // nloc, :, inod % nloc]
            f[inod] = -3 * config.mu_f * torch.exp(-x_inod[0] - x_inod[1] - x_inod[2])
    elif prob == 'nozzle' and ndim == 3:
        for ibc in range(1):
            bci = bc_node_list[ibc]
            for inod in range(bci.shape[0]):
