...
def _s_rhs_fitf(
    ...
    rhs = rhs.view(nele_f, u_nloc, ndim)
...
def get_residual_or_smooth(
    ...
    brk_pnt = np.asarray(np.arange(0, nnn + 1) / nnn * nele_f, dtype=int)
    u_nloc = sf_nd_nb.disp_func_space.element.nloc
    r0 = r0.view(nele_f, u_nloc, ndim)
    if type(x_rhs) is int:
        r0 += x_rhs
    else:
        x_rhs = x_rhs.view(nele_f, u_nloc, ndim)
        r0 += x_rhs
    for i in range(nnn):
        # volume integral
        ...
        if do_smooth:
            ...
            x_i = x_i.view(nele_f, u_nloc, ndim)
            x_i[idx_in, :] += config.jac_wei * torch.einsum(
                '...ij,...j->...i',
                bdiagK,
                r0.view(nele_f, u_nloc * ndim)[idx_in, :]
    ...
    for color in tqdm(range(1, ncolor + 1), disable=config.disabletqdm):
        ...
        for jdim in range(ndim):
            ...
            for idim in range(ndim):
                RARm[:, idim] += torch.mv(
                    I_cf,
                    mg.vel_pndg_to_p1dg_restrictor(ARm[0:nele_f, :, idim])
                )  # (cg_nonods, ndim)
            for idim in range(ndim):
                # add to value
...
def _gmres_mg_solver(
    ...
    m = config.gmres_m
    v_m = torch.zeros(m + 1, nonods * ndim, device=dev, dtype=torch.float64)
    ...
    while r0l2 > tol and sf_nd_nb.its < config.gmres_its:
        ...
        r0 *= 0
        # get residual
        r0 = get_residual_or_smooth(
            r0, x_i, x_rhs,
            do_smooth=False)
        # apply left preconditioner
        r0 = _um_left_precond(r0)

        beta = torch.linalg.norm(r0.view(-1))
        # beta = torch.linalg.norm(r0)
        v_m[0, :] += r0.view(-1) / beta
        ...
        for j in tqdm(range(0, m), disable=config.disabletqdm):
            ...
            w = _um_left_precond(w)
            for i in range(0, j + 1):
                h_m[i, j] = torch.linalg.vecdot(w.view(-1), v_m[i, :])
                w -= h_m[i, j] * v_m[i, :]

            h_m[j + 1, j] = torch.linalg.norm(w.view(-1))
            v_m[j + 1, :] += w.view(-1) / h_m[j + 1, j]
            sf_nd_nb.its += 1
...
def _um_left_precond(x_i):
    """
    do one multi-grid V cycle as left preconditioner
    """
    u_nloc = sf_nd_nb.disp_func_space.element.nloc
    total_no_dofs = nele_f * u_nloc * ndim

    r0 = torch.zeros(nele_f, u_nloc, ndim, device=dev, dtype=torch.float64)
